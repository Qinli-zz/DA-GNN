...
class Model(nn.Module):
    ...
    def forward(self, j_inputs, b_inputs, last_state):
        # Encoder
        ...
        for step in range(self.target_len):
            ...
            H = H.permute(0, 2, 1).contiguous()
            r = torch.sigmoid(self.input_r(last_state) + self.hidden_r(H))
            z = torch.sigmoid(self.input_i(last_state) + self.hidden_i(H))
            n = torch.tanh(self.input_n(last_state) + r * self.hidden_h(H))
            H = (1 - z) * n + z * H
