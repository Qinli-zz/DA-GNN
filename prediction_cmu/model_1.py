...
class GCN(nn.Module):
    def __init__(self, in_channels, A):
        super().__init__()
        w = nn.Parameter(torch.ones(A.size())).cuda("cuda:0")
        b = nn.Parameter(torch.ones(A.size())).cuda("cuda:0")
        self.A = A*w + b
...
class Update(nn.Module):
    def __init__(self, in_channels, out_channels, A_s, A_t):
        super().__init__()
        w_s = nn.Parameter(torch.ones(A_s.size())).cuda("cuda:0")
        b_s = nn.Parameter(torch.ones(A_s.size())).cuda("cuda:0")
        self.A_s = A_s*w_s + b_s

        w_t = nn.Parameter(torch.ones(A_t.size())).cuda("cuda:0")
        b_t = nn.Parameter(torch.ones(A_t.size())).cuda("cuda:0")
        self.A_t = A_t*w_t + b_t
        # Updating functions
