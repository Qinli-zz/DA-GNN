...
if __name__ == '__main__':
    ...
    learning_rate = 0.001
    step = np.arange(200, epoch, 200)    # steps for adjusting learning rate
    ...
    j_states_all, j_last_state_all, j_last_sec_state_all, targets_all = train_sample(normed_train_dict,
                                                                     3200,
                                                                     source_seq_len,
                                                                     target_seq_len,
                                                                     len(dim_use))
    ...
    for action in actions:
        test_data[action] = srnn_sample(normed_test_dict,
                                        action,
                                        source_seq_len,
                                        target_seq_len,
                                        len(dim_use))
    ...
    for epo in range(epoch):
        for n in range(100):
            ...
            j_states = j_states_all[int(batch_size*n): int(batch_size*n+batch_size)]
            j_last_state = j_last_state_all[int(batch_size * n): int(batch_size * n + batch_size)]
            j_last_sec_state = j_last_sec_state_all[int(batch_size * n): int(batch_size * n + batch_size)]
            targets = targets_all[int(batch_size * n): int(batch_size * n + batch_size)]
        ...
        all_errors = []
        for action in actions:
            j_states, j_last_state, j_last_sec_state, targets = test_data[action]
